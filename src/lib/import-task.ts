...
export async function runImportTask(
  ...
  const config = await prisma.openlistConfig.findUnique({
    where: { id: configId },
  });
  if (!config) {
  ...
  if (audioItems.length > 0 && groups.size === 0) {
    const inferredTitle =
      sanitizedTitle ?? path.split("/").filter(Boolean).pop() ?? "未命名专辑";
  ...
  const totalEpisodes = results.reduce(
    (sum, res) => sum + (res.newEpisodes || 0),
    0,
  );
