...
const GENRE_MAP_EN_ZH: Record<string, string> = {
    ...
    "Kids": "儿童", "News": "新闻", "Reality": "真人秀", "Sci-Fi & Fantasy": "科幻奇幻",
    "Soap": "肥皂剧", "Talk": "脱口秀", "War & Politics": "战争政治"
};
...
async function tmdbGet<T>(path: string): Promise<T> {
    ...
    return tmdbCircuitBreaker.execute(async () => {
        ...
        if (!res.ok) {
            ...
            throw new Error(`TMDB API 错误 ${res.status}: ${TMDB_BASE}${path.split("?")[0]}`);
        }
...
export async function searchTmdbMetadata(title: string): Promise<TmdbMetadata | null> {
    ...
    return queue.add(async () => {
        try {
            ...
            const rawDate = isMovie ? detailResp.release_date : detailResp.first_air_date;
            const year = typeof rawDate === 'string' ? new Date(rawDate).getFullYear() : null;

            // 提取导演/制片人 (Crew)
            let director = "";
            let cast: string[] = [];

            if (detailResp.credits && typeof detailResp.credits === 'object' && detailResp.credits !== null) {
                const credits = detailResp.credits as Record<string, unknown>;

                if (isMovie) {
                    const dirObj = Array.isArray(credits.crew) ? credits.crew.find((c: Record<string, unknown>) => c.job === "Director") : null;
                    if (dirObj) director = String(dirObj.name);
                } else {
                    const creatorObj = (Array.isArray(detailResp.created_by) ? detailResp.created_by[0] : null) ||
                        (Array.isArray(credits.crew) ? credits.crew.find((c: Record<string, unknown>) => c.job === "Executive Producer" || c.job === "Producer") : null);
                    if (creatorObj) director = String(creatorObj.name);
                }

                // 提取前 5 个主演 (Cast)
                if (Array.isArray(credits.cast)) {
                    cast = credits.cast.slice(0, 5).map((c: Record<string, unknown>) => String(c.name));
                }
            }
            ...
            const result = {
                ...
                posterUrl: detailResp.poster_path ? `${TMDB_IMAGE_BASE}${detailResp.poster_path}` : null,
