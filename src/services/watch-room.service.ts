...
export class WatchRoomService {
  ...
  static async createRoom(input: CreateRoomInput): Promise<RoomInfo> {
    ...
    if (activeRoomCount >= MAX_ACTIVE_ROOMS_PER_USER) {
      throw AppError.badRequest(
        `最多同时创建 ${MAX_ACTIVE_ROOMS_PER_USER} 个房间，请先关闭旧房间`,
      );
    }
    ...
    if (!episode || episode.seriesId !== input.seriesId) {
      throw AppError.notFound("指定的剧集或集数不存在");
    }
  ...
  static async joinRoom(
    ...
    if (!room) {
      throw AppError.notFound("房间不存在");
    }

    if (room.status === ROOM_STATUS.CLOSED) {
      throw AppError.badRequest("房间已关闭");
    }
    ...
    if (room.members.length >= MAX_MEMBERS_PER_ROOM) {
      throw AppError.badRequest(`房间已满（最多 ${MAX_MEMBERS_PER_ROOM} 人）`);
    }
   ...
   * 获取房间信息
   */
  static async getRoom(roomId: string): Promise<RoomInfo> {
    const room = await prisma.watchRoom.findUnique({
      where: { id: roomId },
      include: {
        members: {
  ...
  static async cleanupExpiredRooms(): Promise<number> {
    ...
    if (result.count > 0) {
      console.info(`[WatchRoom] 清理了 ${result.count} 个过期房间`);
    }
